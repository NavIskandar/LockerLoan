...
if (isset($_GET['op']) AND $_GET['op'] == 'delete' AND isset($_GET['itemID'])) {
    ...
    // update
    $delete= $dbs->query('DELETE FROM locker_pattern WHERE pattern_prefix="'.$suffix.'"');
    if ($delete) {
      echo 'scs';
    } else {
...
<div class="menuBoxInner masterFileIcon">
    ...
    <div class="sub_section">
      <div class="btn-group">
      <a href="<?php echo MWB; ?>locker_loan/item_code_pattern.php" class="list btn btn-default"><i class="glyphicon glyphicon-list-alt"></i>&nbsp;<?php echo __('Pattern List'); ?></a>
          <a href="<?php echo MWB; ?>locker_loan/pop_pattern.php?in=master" class="notAJAX btn btn-default openPopUp notIframe"><i class="glyphicon glyphicon-plus"></i>&nbsp;<?php echo __('Add New Pattern'); ?></a>
      </div>
    </div>
</div>
</fieldset>
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    $pattern_q = $dbs->query('SELECT pattern_prefix, pattern_zero, pattern_suffix FROM locker_pattern');
    if ($pattern_q->num_rows > 0) {
        ...
        // no data
        echo 'No Patternt available. <a class="notAJAX btn btn-primary openPopUp notIframe" href="'.MWB.'locker_loan/pop_pattern.php?in=master" height="420px" title="'.__('Add new pattern').'">
