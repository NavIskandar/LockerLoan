...
if (isset($_GET['op']) AND $_GET['op'] == 'insert') {
  ...
  $check = $dbs->query('SELECT pattern_prefix FROM locker_pattern WHERE pattern_prefix="'.$_prefix.'"');
  // Check
  if ($check->num_rows > 0) {
    echo "Pola sudah ada!";
    exit();
  ...
  // Set query
  $insert = $dbs->query("INSERT INTO locker_pattern (pattern_prefix, pattern_zero, pattern_suffix, input_date) VALUES('".$_prefix."', '".$count_zero."', '".$_suffix."', '".date('Y-m-d')."')");
  // Check
  if ($insert) {
    echo 'scs';
  }
          ...
          if (msg == 'scs') {
            ...
            <?php
              if (isset($_GET['in']) AND $_GET['in'] == 'master'){
                echo 'parent.$(\'#mainContent\').simbioAJAX(\''.MWB.'locker_loan/item_code_pattern.php\');';
              } else {
